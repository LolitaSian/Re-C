...
- 按照编号排序：按照职工编号，进行排序，排序规则由用户指定清空所有文档：清空文件中记录的所有职工信息（清空前需要再次确认，防止误删）

## p3演讲比赛流程管理系统

**比赛规则**：
- 学校举行一场演讲比塞，共有12个人参加。比赛共两轮，第一轮为淘汰塞，第二轮为决賽。
- 每名选手都有对应的编号，如10001-10012比赛方式：分组比赛，每组6个人；
- 第—轮分为两个小组，整体按照选手编号进行抽签后顺序演讲。
- 十个评委分别给每名选手打分，去除最高分和最低分，求的平均分为本轮选手的成绩当小组演讲忘后，淘汰组内排名最后的三个选手，前三名晋级，进入下一轮的比赛。
- 第二轮为决赛，前三名胜出
- 每轮比塞过后需要显示晋级选手的信息

**程序功能**：
- 开始演讲比：完成整届比襄的流程，每个比襄阶段需要给用户一个提冠，用户按任意键后继续下一个阶段
- 查看往届记录：查看之前比襄前三名结果，每次比塞都会记录到文件中，文件用csV后缀名保存
- 清空比赛记录：将文件中数据清空退出比塞程序：可以退出当前程序
...
**容器**：将运用最广泛的一些数据结构表述出来，分为序列式容器和关联式容器

- 序列式容器：强调值的排序，序列式容器中的每个元素均有固定的位置
- 关联式容器：二叉树结构，各元素之间没有严格的物理上的顺序关系

**算法**：有限的步骤解决逻辑或数学上的问题，分为质变算法和非质变算法

- 质变算法：运算过程中更改区间内容，比如拷贝、替换
- 非质变算法：运算过程不会更改区间元素的内容，比如查找、遍历

**迭代器**：

提供一种方法，使之能够依顺序寻访某容器的各个元素，而又无需暴露该容器的内部表示方法，每个容器都有自己专属的迭代器。迭代器的使用常类似于指针，初学阶段我们可以先理解为迭代器指针。常用容器中迭代器种类为双向迭代器或随机访问迭代器。
...
| 输入迭代器     | 对数据的只读访问                       | 支持++、==、!=                    |
| 输出迭代器     | 对数据的只写访问                       | 支持++                            |
| 向前迭代器     | 读写操作，并能向前推进的迭代器         | 支持++、==、!=                    |
| 双向迭代器     | 读写操作，并能向前和向后操作           | 支持++、--                        |
| 随机访问迭代器 | 读写操作，可以以跳跃的方式访问任意数据 | 支持++、--、<、>、<=、>=、-n、[n] |
...
`vector<数据类型>::iterator 迭代器名称 = 变量名.begin()`

`vector<数据类型>::iterator 迭代器名称 = 变量名.end()`

### 12.3 string

string本质上是C++的一个类

**string和char\*的区别**：char\*是指针，string是类，内部封装了char\*，是一个char*的容器
...
- `string& operator +=(const char* s)`重载+=操作
- `string& operator +=(const char c)`重载+=操作
- `string& operator +=(const string& s)`重载+=操作
- `string& append(const char* s)`把字符串s拼接到字符串结尾
